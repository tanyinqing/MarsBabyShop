...
import com.xlw.babyshop.parser.BulletinParser;
import com.xlw.babyshop.tasks.NetCheckAsyncTask;
import com.xlw.babyshop.ui.adapter.BulletinLvAdapter;
...
public class BulletinActivity extends BaseActivity implements VolleyUtil.ResponseCallback{
    ...
    private List<BulletinModel> serverResponseDataList;   // 从服务器返回的数据集合
...
//        }
        ...
        netCheckTask.setCallBack(new NetCheckAsyncTask.CallBack() {
            ...
            public void handleNetCheckResponse(Boolean aBoolean) {
                if (aBoolean) {
                    ...
                    // 如果无法联上互联网
                    ToastUtil.showLongMsg(BulletinActivity.this, "无法连接互联网");
                }
            }
        });
    ...
    private void requestDataFromServer(){
        ...
        BaseJSONParser<List<BulletinModel>> jsonParser = new BulletinParser();
        // 建立请求对象模型
        String requestUrl = this.getResources().getString(R.string.topic);
        // 向服务器提交的分页数据
        ...
        // 向服务器端请求版本信息
        // "http://10.0.3.2:8080/MarsShop/" + "topic"
...
//        volleyUtil.requestString(reqDataUrl);       // 向服务器异步请求版本信息
    ...
    private void setListener() {
        ll_bulletin_prom.setOnItemClickListener(new AdapterView.OnItemClickListener(){
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                BulletinModel vo = (BulletinModel)ll_bulletin_prom.getItemAtPosition(position);
    ...
    public void responseString(String responseText) {
        ...
        try {
            // 解析
            serverResponseDataList = (List<BulletinModel>)requestModel.jsonParser.parseJSON(responseText);
        ...
        if (serverResponseDataList != null && serverResponseDataList.size()>0) {
            ...
            BulletinLvAdapter bulletinAdaper = new BulletinLvAdapter(BulletinActivity.this, serverResponseDataList);
